...
try {
    ...
    Write-Host "`n[1/4] Installing luatools.js..." -ForegroundColor White
    ...
    Write-Host "[2/4] Installing en.json..." -ForegroundColor White
    $localesPath = "C:\Program Files (x86)\Steam\plugins\luatools\backend\locales"
    if (!(Test-Path $localesPath)) { New-Item -ItemType Directory -Path $localesPath -Force | Out-Null }
    Invoke-WebRequest -Uri "https://raw.githubusercontent.com/CONNORKURD/Alpha-1/refs/heads/main/en.json" `
        -OutFile "$localesPath\en.json" -UseBasicParsing
    Write-Host "      Done!" -ForegroundColor Green

    # 3. plugin.json → luatools root folder
    Write-Host "[3/4] Installing plugin.json..." -ForegroundColor White
    ...
    Write-Host "[4/4] Installing luatools-icon.png..." -ForegroundColor White
